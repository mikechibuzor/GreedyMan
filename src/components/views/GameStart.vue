<template>
  <div class="contt flex bg-primary-color  items-center justify-center  h-screen w-screen pt-8 pb-12 px-10">
    <div class="content h-height-80  w-full grid grid-cols-3 overflow-hidden  ">
        <section class="col-span-1 h-full pr-6">
            <img class="w-full h-full" :src="require('../../assets/GreedyHunter/Group 10.svg')" alt="background_mesh" />
        </section>
        <main class="col-span-1 flex flex-col items-center pt-6">
            <div class="avatar h-40 w-40 mb-8">
                <svg class="w-full h-full" viewBox="0 0 35 38" fill="none" xmlns="http://www.w3.org/2000/svg">
...
>>>>>>> e8cccfc01daf3508cf5293d3c6648a58a4347888
            ...
            <form action="#" @submit.prevent="startButton" class="flex flex-col items-center justify-center">
                ...
                <p v-if="showError" class="text-red-500 text-sm text-center">{{ errorMsg }}</p>
                <label for="grid-number" class="text-2xl text-secondary-pinklike mr-4">Game grid</label>
...
<<<<<<< HEAD
    ...
    computed:{
        ...
        endMoves(){
            return this.$store.getters['getEndMoves'];
        },
        timeSpent(){
            return this.$store.getters['getTimeSpent'];
        },
        startButtonText(){
            return this.$store.getters['getStartButtonText'];
        },
        win(){
